...
const PatientContacts = () => {
  ...
  const handleDeleteContact = async () => {
    const res = await dispatch(deleteContactAction(selectedContact._id));
            ...
            {patient?.contact?.map((contact, idx) => (
              <tr key={idx} className="border-b">
                ...
                <td className="text-xs py-3">
                  {contactType?.find((c) => c._id === contact.contacttype)
                    ?.name || "-"}
                </td>
                <td className="text-xs py-3">{contact?.firstname}</td>
                <td className="text-xs py-3">{contact?.lastname}</td>
                <td className="text-xs py-3">{contact?.address}</td>
                <td className="text-xs py-3">{contact?.phone.join(",")}</td>
                <td className="text-xs py-3">{contact?.email}</td>
